...
@login_required
def index():
    selected_date_str = request.form.get("selected_date") or request.args.get(
        "selected_date"
    )
    ...
    if selected_date:
        ...
        order_details = (
            db.session.query(
                ...
                Order.note,
                func.coalesce(image_count_subquery.c.image_count, 0).label(
                    "image_count"
                ),
            )
            ...
            .outerjoin(
                image_count_subquery, Order.id == image_count_subquery.c.order_id
            )
            .filter(Order.date == selected_date)
            .group_by(Order.id)
            .outerjoin(Customer, Order.customer == Customer.customer)
            .order_by(
                Customer.priority.asc(),
                Customer.packing.asc(),
                Order.product.asc(),
                "fish_size",
            )
            .all()
        ...
        for order in order_details:
            ...
            if order[9] == "Lohi":
                ...
                if float(order[6]) < float(order[5]) * float(
                    os.environ.get("COMPLETION_THRESHOLD", 0.9)
                ):
                    box_info_unfinished = np.array(
                        calculate_salmon_box(float(order[5]))
                    )
                    details[key_name] = details[key_name] + np.vstack(
                        [box_info_total, box_info_unfinished]
                    )
                else:
                    details[key_name] = details[key_name] + np.vstack(
                        [box_info_total, [0, 0]]
            ...
            for subcategory, items in subcategories.items():
                ...
                for item, values in items.items():
                    row = {
                        ...
                        "merge_info": {
                            "category_rowspan": category_rowspan_tracker.get(
                                category, 0
                            ),
